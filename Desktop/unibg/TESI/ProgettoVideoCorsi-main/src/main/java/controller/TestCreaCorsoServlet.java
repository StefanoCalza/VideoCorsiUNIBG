...
@WebServlet("/testcreacorsoservlet")
public class TestCreaCorsoServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private Connection connection = null;

    public TestCreaCorsoServlet() {
        super();
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int maxCourseId;
        try {
            System.out.println("[TestCreaCorsoServlet] Parametri ricevuti: Coursename='" + request.getParameter("Coursename") + "', Coursedescription='" + request.getParameter("Coursedescription") + "'");
            TransactionManager.beginTransaction(connection);
            maxCourseId = courseDao.getMaxCourseId();
            System.out.println("[TestCreaCorsoServlet] maxCourseId attuale: " + maxCourseId);
            //insert the new course
            courseDao.insertCourse(maxCourseId + 1, request.getParameter("Coursename"), request.getParameter("Coursedescription"));
            System.out.println("[TestCreaCorsoServlet] Corso inserito con id: " + (maxCourseId + 1));
            ...
            System.out.println("[TestCreaCorsoServlet] Errore SQL: " + e.getMessage());
